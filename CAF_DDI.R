...
lag.trans <- function(x, lag){
  # x: vector, matrix, data frame
  # lag: lag length, numeric vector
  # length(lag) = ncol(x) = 1
  if(lag == 0){
    return(x)
  } else if(lag > 0){
    y <- c(rep(0, lag), x[1:(length(x) - lag)])
    return(y)
  } else if(lag < 0){
    y <- c(x[(-lag + 1):length(x)], rep(0, -lag))
    return(y)
  }
}
...
modif <- function(pred, resp, data, tm.prmt){
  ...
  # No Transformation: tm.prmt = c(NA,NA,NA,NA,0)
  # Holiday Scaling:   tm.prmt = c(1,NA,NA,NA,0)
  # Only Lag:          tm.prmt = c(NA,NA,NA,NA,<>0)
  # Original S/Power:  tm.prmt = c(  ,  ,  ,  ,0)
  # Lag + S/Power:     tm.prmt = c(  ,  ,  ,  ,<>0)
  ...
  if(lagl != 0){
    data[[pred]] <- lag.trans(data[[pred]], lagl)
  }
  if(is.na(co.r)){
    df[[pred]] <- data[[pred]]
  }else if(co.r == 1){
    df[[pred]] <- hmodif(pred, resp, data)
  }else if(is.na(pc.r)){
...
questions <- function(index, pred = NULL, df = NULL, coef = NULL, opt = NULL){
  ...
  # index = 11
  q2.2.1.1_pre <- function(){
    repeat{
      lagnum <- readline("| Please specify the lag length (integer, 0 stands for no lag needed): ")
      cat("\n")
      if(!all(strsplit(lagnum, split = "")[[1]] %in% c(as.character(0:9), "-"))){
        message("| Only integer is acceptable! \n")
      } else if(abs(as.numeric(lagnum)) >= nrow(e$data) - e$st.row + 1){
        message("| The lag is longer than the variable length! \n")
      } else break
    }
    return(as.numeric(lagnum))
  }
...
rebuild <- function(resp, data, st.row) {
  ...
  for(i in 1:nrow(prmt.alive)){
    ...
    df0 <- modif(pred, resp, df0, c(co.r, pc.r, sc.1, sc.2, lagl))
  }
  e$df0 <- df0
...
final.output <- function(resp, data, rawdata, tvar, fit, prmt, aic = FALSE) {
  ...
  #       variable, trans.meth, co.r, sc.1, sc.2, pc.r, lagl, status
  ...
  # 1.Transformation (trans.meth, co.r, sc.1, sc.2, pc.r, lagl)
  ...
  names(model) <- c("variable", "coefficient", "co.r", "pc.r",	"sc.1", "sc.2", 
                    "lag", "std.error", "t.value", "p.value", "vif", "contribution")
  ...
  #       variable, trans.meth, co.r, sc.1, sc.2, pc.r, oth, status
	for(var in effivar){
		...
		if(lagl != 0){
		  effidf[[var]] <- lag.trans(effidf[[var]], lagl)
		}
...
StepReg <- function(){
  ...
  repeat{
    ...
    if (!endstr %in% c(".csv", "xlsx", ".xls")){
      ...
      message('| There are ', nrow(e$data), ' observations and ', 
              ncol(e$data), ' variables in the raw dataset.\n')
      ...
      if(sum(num.na)!=0){
        ...
        message("| ", paste(names(e$data)[which(num.na != 0)], sep = ", "), ". \n")
  ...
  repeat{
    ...
    if(a_nm == 1){ # opt1. Add a new predictor
      repeat{
        ...
        repeat{
          # First! Ask lag length!
          e$lagl <- questions(11)
          e$df.templag <- modif(e$pred, e$resp, e$df0, c(rep(NA, 4), e$lagl))
          # STEP 2.2.1.1 transformation method
          e$tm <- questions(3, pred = e$pred)
          if(e$tm %in% 1:3){ # 0-no trans; 1-s curve; 2-power curve; 3-1 or 2
            e$tm.prmt <- c(recom(e$pred, e$resp, e$df.templag, e$tm, e$fit1, e$st.row), e$lagl)
            e$df.temp0 <- modif(e$pred, e$resp, e$df0, e$tm.prmt)
          } else if(e$tm == 0){
            e$tm.prmt <- c(rep(NA, 4), e$lagl)
            e$df.temp0 <- modif(e$pred, e$resp, e$df0, e$tm.prmt)
          } else if(e$tm == 4){
          ...
          repeat{
            ...
            if(e$a_stsf == 1){ # opt1 change transformation prmt
              if(e$tm == 3){
                e$tm.prmt <- if(questions(5) == 1){
                    c(questions(8), e$lagl)
                  }else{
                    c(questions(7), e$lagl)
                  }
              } else if(e$tm == 1){
                e$tm.prmt <- c(questions(8), e$lagl)
              } else if(e$tm == 2){
                e$tm.prmt <- c(questions(7), e$lagl)
              }
        ...
        if(e$a_stsf == 4){ # satisfied
          ...
          names(prmt.line) <- c("variable", "co.r", "pc.r", "sc.1", "sc.2", "lagl", "status")
      ...
      if(e$a_conf == "Y"){
        ...
        e$df1 <- e$df0[e$st.row:nrow(e$df0), ]
        e$prmt$status[which((e$prmt$variable == e$pred) & (e$prmt$status == "alive"))] <- "dead"
