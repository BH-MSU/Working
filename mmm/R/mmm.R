...
#' @param co.rate The carry over rate. The value should be between 0 and 1.
...
#' @param x A numeric vector to be transformed.
#' @param lamda1,lamda2 Two parameters of the s-curve.
#' @return The transformed numeric vector.
sc <- function(x, lamda1, lamda2) 1 - exp(-lamda1 * x^lamda2)

#' Carryover + S-curve
#'
#' Formula: transformed_x <- sc(co(x, co.rate), lamda1, lamda2)
#' @param x A numeric vector to be transformed.
#' @param co.rate The carry over rate. The value should be between 0 and 1.
#' @param lamda1,lamda2 Two parameters of the s-curve.
#' @return The transformed numeric vector.
#' @seealso \code{\link{co}} and \code{\link{sc}}
cs <- function(x, co.rate, lamda1, lamda2){
  x <- 1 - exp(-lamda1 * co(x, co.rate)^lamda2)
}

#' Carryover + Power-curve transformation
#'
#' x <- pc(co(x, co.rate), exponent)
#' @param x A numeric vector to be transformed.
#' @param co.rate The carry over rate. The value should be between 0 and 1.
#' @param exponent The exponent rate.
#' @return The transformed numeric vector.
...
#'
#' Update a linear regression model by adding or removing one new variable.
#' Create a model if there is not one at all.
#' @param data A data.frame contain both predictor and respondent variables.
#' @param resp A character, name of respondent variable included in the data.frame.
#' @param fit A linear regression model. The default value is NULL.
#' @param action 1 or -1. 1 means add a predictor; -1 means delete a predictor. The default value is 1.
#' @param pred A character, name of predictor variable included in the data.frame.
...
#' Recommend best set of data transforming parameters
#'
...
#' @param pred A character, name of the predictor to be inserted to the model.
#' @param resp A character, name of the response variable in the model.
#' @param df A data.frame contains the predictor and response variables.
#' @param type The transformation method for the predictor. 1 means test all combinations of parameters of carryover & s-curve; 2 means test all combinations of parameters of carryover & power curve; 3 means do both of the actions above
#' @param fit A linear regression model. The default value is NULL.
#' @param st.row Numeric, number of modeling starting row.
#' @return A numeric vector of length 4, contains the recommended parameters for transformation.
recom <- function(pred, resp, df, type, fit = NULL, st.row){
  ...
  cstrans <- function(x){
    cs.mat <- t(plyr::mdply(expand.grid(co.rate = co.range,
                                  lamda1 = lamda1.range,
                                  lamda2 = lamda2.range),
                      cs, x))
    # return(cs.mat)
  ...
  #---------------------------------
  cptrans <- function(x){
    cp.mat <- t(plyr::mdply(expand.grid(co.rate = co.range,
                                  exponent = pc.range),
                      cp, x))
    # return(cp.mat)
  }
...
#' Holiday variable transformation
#'
#' Holiday vector equals to 1 on the current period of the holiday and 0 on other periods. The variable is replaced by the transformed data according to the current scale of the response variable.
#' @param pred A character, name of variable to be transformed.
#' @param resp A character, name of the response variable of the model.
#' @param data A data frame to store the variable transformed (original variable data will be replaced).
#' @return The complete data.frame with selected variable transformed.
...
#' @param resp A character, name of the response variable of the model.
#' @param data A data frame to store the variable transformed (original variable data will be replaced).
#' @param tm.prmt A numeric vector of length 4 containing transformation parameters.
#' @return The complete data.frame with selected variable transformed.
...
#'
#' The variable is replaced by the transformed data using carry over and S-curve or power-curve transformation.
#' @param index Number of question to be used.
#' @param pred A character, name of variable to be transformed.
#' @param df A data.frame to store the variable transformed (original variable data will be replaced).
#' @param coef A data.frame, form of fit$coef.
#' @param opt The option for the upper question.
#' @return The answer option for the question.
  ...
  q2.2.3.2 <- function(){
    ...
    repeat{
      ...
      if(!toupper(stepaic) %in% c("Y","N")){
        ...
        aic <- ifelse(toupper(stepaic) == "Y", TRUE, FALSE)
        break
      }
    }
    return(aic)
...
#' @param fit1,fit2 Two models that are needed to be compared.
#' @param p.cons The critical point, when higher than which warnings appear.
#' @return Warnings printed on the screen.
...
#' Rebuild model
#'
#' After importing the file of the variables and correspondent transforming parameters, rebuild the model of the specified response variable.
#' @param resp A character, name of response variable.
#' @param data Raw data.frame without modification.
#' @param st.row The number of modeling start row.
#' @return The transformed data and base model to the package working environment.
  ...
  repeat{
    ...
    if (!endstr %in% c(".csv", "xlsx", ".xls")){
      ...
      if(endstr == ".csv"){
        ...
        sht.names <- names(xlsx::getSheets(xlsx::loadWorkbook(prmt.file)))
        repeat{
        ...
        e$prmt <- xlsx::read.xlsx(prmt.file, sheetName = sht.name2, stringsAsFactors = F)
        # names(e$prmt) <- c("variable", "co.r", "pc.r", "sc.1", "sc.2", "status")
      }
      break
    }else if(!file.exists(prmt.file)){
...
#' @param ... The ggplots.
#' @param nrow Number of row the ggplots are wanted to be arranged.
#' @param ncol Number of column the ggplots are wanted to be arranged.
#' @param as.table Logical.
#' @return Plots.
arrange_ggplot2 <- function(..., nrow=NULL, ncol=NULL, as.table=FALSE) {
  vp.layout <- function(x, y) grid::viewport(layout.pos.row=x, layout.pos.col=y)
  dots <- list(...)
  n <- length(dots)
...
#' Loop output
#'
#' Print summary and some useful statistics of the temporary model to the screen.
#' @param resp A character, name of response variable.
#' @param data The data.frame of the modified data.
#' @param fit The temporary model.
#' @param pred A character, name of predictor.
#' @param tvar A character, name of time variable, used for plotting.
...
loop.output <- function(resp, data, fit, pred, tvar) {
  ...
  scat_res <- ggplot(df_to_plot, aes(x = x_axis, y = res)) +
  ...
  hist_res <- ggplot(df_to_plot, aes(x = res))                      +
    geom_histogram(stat = "bin", fill = "orange",  # stat = "identity"
  ...
  fit_vs_act <- ggplot(fit_to_plot, aes(x_axis))                         +
    geom_line(ggplot2::aes(y = act.resp), colour = "darkgrey", size = 1) +
    geom_line(ggplot2::aes(y = fit.resp), colour = "red", size = 1)      +
...
#'
#' Print summary and some useful statistics of the temporary model to the screen.
#' Output necessary data to external files such as *.csv files for rebuilding models.
#' @param resp A character, name of response variable.
#' @param data The data.frame of the modified data.
#' @param tvar A character, name of time variable, used for plotting.
#' @param fit The temporary model.
#' @param prmt A dataframe including names of predictors, transforming parameters and variable status("alive" or "dead").
#' @param aic Logical value. TRUE for need AIC step session, FALSE for not. The default value is FALSE.
#' @return Print output to the screen. External files such as *.csv files or *.xlsx files.
#' @import xlsx rJava
...
#' @return Print out top 20 correlation rate and export correlation rate of all variables to *.csv file.
...
#' StepReg
#'
#' The structure function which cover the whole functionality using all self-built functions.
#' @param ... Nothing necessary.
#' @return Model and related files.
#' @export
#' @import xlsx lubridate
StepReg <- function(...){
  # STEP 0.5
  ...
  message("| Please put your necessary data file under this directory. ")
  message("| All the related files will all be outputed to this directory as well. \n")
  # STEP 1.2
  # Read Data
  repeat{
        ...
        repeat{
          ...
          repeat{
            ...
            if(e$a_stsf == 1){ # opt1 change transformation prmt
              if(e$tm == 3){
                e$tm.prmt <- if(questions(11) == 1) questions(8) else questions(7)
              } else if(e$tm == 1){
